...
// 2) GOOGLE_APPLICATION_CREDENTIALS = JSON string OR path to key file (for local dev / legacy)

const bqOptions = {};
if (process.env.GCP_PROJECT_ID) {
  bqOptions.projectId = process.env.GCP_PROJECT_ID;
}

if (process.env.GOOGLE_SERVICE_ACCOUNT_JSON) {
  // Preferred for Railway: entire JSON stored in one env var
  try {
    ...
    console.error(
      'Failed to parse GOOGLE_SERVICE_ACCOUNT_JSON, falling back to GOOGLE_APPLICATION_CREDENTIALS if set:',
      err.message
    );
  }
}

if (!bqOptions.credentials && process.env.GOOGLE_APPLICATION_CREDENTIALS) {
...
if (!bqOptions.credentials && !bqOptions.keyFilename) {
  console.warn(
    'WARNING: No GCP credentials configured. BigQuery calls will fail until you set GOOGLE_SERVICE_ACCOUNT_JSON or GOOGLE_APPLICATION_CREDENTIALS.'
  );
}
...
function buildSageAuthUrl(state) {
  ...
  let baseUrl =
    process.env.SAGE_AUTH_URL || 'https://oauth.accounting.sage.com/authorize';
